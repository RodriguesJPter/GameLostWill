...
public class InimigoSoldado extends Entidade {
	...
	public InimigoSoldado(int x, int y, int width, int height, BufferedImage sprite) {
		...
		inimigoSLeftA = new BufferedImage[4];
		inimigoSRightA = new BufferedImage[4];
		...
		for(int i = 0; i < 5; i++) {
			inimigoSLeftP[i] = Game.sprite.getSprite(784 - (i*48) , 1920, 48, 64);
		}

		for(int i = 0; i < 5; i++) {
			inimigoSRightP[i] = Game.sprite.getSprite(592 + (i*48) , 2000, 48, 64);
		}
		for(int i = 0; i < 4; i++) {
			inimigoSLeftA[i] = Game.sprite.getSprite(224 - (i*64) , 2000, 64, 64);
		}

		for(int i = 0; i < 4; i++) {
			inimigoSRightA[i] = Game.sprite.getSprite(32 + (i*64) , 1920, 64, 64);
		}
	...
	public void lifeBar(Graphics g) {
        ...
        g.drawString("Soldado Sol", inventarioXB, inventarioYB);
	}
	...
	public void animations(Graphics g) {
		if (direcaoAtual == esquerda && movimentacao == 1) {
			g.drawImage(inimigoSRight[index], this.getX()-Camera.x - 15, this.getY()-Camera.y - 40, null);
		}
		...
		if (direcaoAtual == direita && movimentacao == 1) {
			g.drawImage(inimigoSLeft[index], this.getX()-Camera.x - 15, this.getY()-Camera.y - 40, null);
		}
		...
		if (direcaoAtual == direita && movimentacao == 0 && atacando == 1) {
			g.drawImage(inimigoSRightA[indexA], this.getX()-Camera.x - 36 , this.getY()-Camera.y - 48, null);
		}
